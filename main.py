...
try:
    c.execute('ALTER TABLE server_config ADD COLUMN value TEXT')
    conn.commit()
except Exception:
    pass
...
def embed(title, desc, color=None):
    ...
    if color is None:
        ...
        color = int(raw) if raw and raw.isdigit() else 0x5865F2
    ...
    thumbnail = cfg.get('embed_thumbnail', '')
    if thumbnail:
...
async def sethouse(ctx, house_name: str, role: discord.Role):
    ...
    await ctx.send(embed=embed(
        "🏠 House Linked",
        f"**{house_name.capitalize()}** is now linked to {role.mention}.\nUse `!assign @user {house_name.capitalize()}` or `!assign @user @role` to add members.",
        color=0x57F287
    ))
